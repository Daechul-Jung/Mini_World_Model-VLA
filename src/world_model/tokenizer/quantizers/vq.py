...
The quantizer is its own registry slot because codebook collapse -- a healthy
reconstruction loss achieved with 5% of the codebook in use -- is the most common
stage-A failure, and the fix is almost always a different quantizer rather than a
different encoder. Track `codebook_usage` and `perplexity`, not just PSNR.
See `research/003_quantizer_alternatives.md`.
"""
...
@QUANTIZERS.register("vq", paper="van den Oord et al., 2017", status="baseline")
